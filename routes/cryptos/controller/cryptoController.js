...
async function cryptosGet(req, res) {
    ...
    console.log('');
    try {
        let payload = await Crypto.find(req.body);
...
async function cryptoCreate(req, res) {
    ...
    if (!cryptoFound) {
        const cryptoCreated = new Crypto({
            nameCrypto,
            nameSymbol,
            logoImgSrc,
            website,
        ...
        let cryptoSaved = await cryptoCreated.save();
        res.json({
...
async function cryptoDelete(req, res) {
    ...
    switch (cryptoDeleted.deletedCount) {
        case 1:
            ...
            break;
        case 0:
            console.log(`You have NOT deleted ${_id}, as it is not found. please check Symbol`)
            res.status(404).json({
